...
- **Web3 Integration:** Uses a library like `ethers.js` or `web3.js` to connect to the user's Ethereum wallet (e.g., MetaMask) and interact with the deployed smart contracts.
- **Responsive Design:** UI will be accessible on various devices (desktop, tablet, mobile).
...
- **Navigation Links:**
  ...
  - Marketplace/Explore NFTs (Optional): View NFTs available for loans or recently liquidated.
...
- For each loan:
  ...
  - "Repay Loan" button
  - "Claim Royalties & Repay" button (visible only if the collateral is a registered Story Protocol IP)
...
_As previously described, with additional notifications for royalty claim successes or failures._
